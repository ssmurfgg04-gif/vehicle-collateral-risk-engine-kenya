...
export async function GET() {
  ...
  if (count > 0) {
    return NextResponse.json({ message: 'Database already seeded', vehicleCount: count });
  }
  ...
  try {
    ...
    const keysian = await db.auctioneer.create({ data: { name: 'Keysian Auctioneers', location: 'Cannon House, Haile Selassie Ave, Nairobi', specialization: 'BANK_REPOSSESSION' } });
    ...
    // ─── STORAGE YARDS ───
    const valuersYard = await db.storageYard.create({ data: { name: 'Valuers Yard Enterprises', location: 'Thika Road, Nairobi', county: 'NAIROBI', phone: '+254721636213' } });
    const startruckYard = await db.storageYard.create({ data: { name: 'Startruck Storage Yard', location: 'Thika Road/Kiambu Road', county: 'NAIROBI', phone: '+254721310340' } });
    const eldoretYard = await db.storageYard.create({ data: { name: 'Eldoret Auction Centre', location: 'Eldoret', county: 'UASIN_GISHU', phone: '+254721681905' } });
    const mombasaYard = await db.storageYard.create({ data: { name: 'Mombasa Matriz Moves', location: 'Mombasa', county: 'MOMBASA', phone: '+254720802646' } });

    // ─── VEHICLES ───
    const v1 = await db.vehicle.create({ data: { normalizedPlate: 'KDA123X', rawPlate: 'KDA 123X', make: 'TOYOTA', model: 'HILUX', variant: 'D/CABIN SR 4WD', year: 2015, color: 'WHITE', normalizedChassis: 'JTEBU3JR3B5045181', rawChassis: 'JTEBU3JR3B5045181', plateCategory: 'PRIVATE', countyCode: 'KD', degreeCentrality: 3.5, clusteringCoeff: 0.8, fraudRingSize: 5 } });
    const v2 = await db.vehicle.create({ data: { normalizedPlate: 'KCA456B', rawPlate: 'KCA 456B', make: 'TOYOTA', model: 'PRADO', variant: 'TXL', year: 2018, color: 'BLACK', normalizedChassis: 'JTEBU5JR8J5301234', rawChassis: 'JTEBU5JR8J5301234', plateCategory: 'PRIVATE', countyCode: 'KC', degreeCentrality: 1.2, fraudRingSize: 1 } });
    const v3 = await db.vehicle.create({ data: { normalizedPlate: 'KBB789Y', rawPlate: 'KBB 789Y', make: 'NISSAN', model: 'X-TRAIL', variant: 'LE 2WD', year: 2020, color: 'SILVER', normalizedChassis: 'JN1TANT31Z0005678', rawChassis: 'JN1TANT31Z0005678', plateCategory: 'PRIVATE', countyCode: 'KB', degreeCentrality: 2.0, fraudRingSize: 2 } });
    const v4 = await db.vehicle.create({ data: { normalizedPlate: 'KAE321M', rawPlate: 'KAE 321M', make: 'MAZDA', model: 'CX-5', variant: '2.0L SPORT', year: 2022, color: 'RED', normalizedChassis: 'JMZKEW11700012345', rawChassis: 'JMZKEW11700012345', plateCategory: 'PRIVATE', countyCode: 'KA', degreeCentrality: 0.5, fraudRingSize: 0 } });
    const v5 = await db.vehicle.create({ data: { normalizedPlate: 'KDF654Z', rawPlate: 'KDF 654Z', make: 'ISUZU', model: 'NPR', variant: 'TRUCK 3.0L', year: 2017, color: 'BLUE', normalizedChassis: 'JALNTF19R77001234', rawChassis: 'JALNTF19R77001234', plateCategory: 'PRIVATE', countyCode: 'KD', degreeCentrality: 2.8, clusteringCoeff: 0.7, fraudRingSize: 4 } });
    const v6 = await db.vehicle.create({ data: { normalizedPlate: 'KAW072Z', rawPlate: 'KAW 072Z', make: 'TOYOTA', model: 'COROLLA', variant: '1.6L GLI', year: 2013, color: 'WHITE', normalizedChassis: '1NXBRU4EXDZ000789', rawChassis: '1NXBRU4EXDZ000789', plateCategory: 'GOVERNMENT', countyCode: 'KAW' } });
    await db.vehicle.create({ data: { normalizedPlate: 'KBA111A', rawPlate: 'KBA 111A', make: 'SUBARU', model: 'FORESTER', variant: '2.0I', year: 2019, color: 'GREEN', plateCategory: 'PRIVATE', countyCode: 'KB' } });
    await db.vehicle.create({ data: { normalizedPlate: 'KCC222C', rawPlate: 'KCC 222C', make: 'HONDA', model: 'CR-V', variant: '1.5T', year: 2021, color: 'GRAY', plateCategory: 'PRIVATE', countyCode: 'KC' } });
    await db.vehicle.create({ data: { normalizedPlate: 'GKA277G', rawPlate: 'GKA 277G', make: 'PEUGEOT', model: '406', variant: '1.8L', year: 2008, color: 'SILVER', plateCategory: 'GOVERNMENT', countyCode: 'GKA' } });
    await db.vehicle.create({ data: { normalizedPlate: 'KEE333E', rawPlate: 'KEE 333E', make: 'MITSUBISHI', model: 'PAJERO', variant: '3.0L GLS', year: 2016, color: 'BLACK', plateCategory: 'PRIVATE', countyCode: 'KE' } });
    ...
    await db.riskCheck.create({ data: { requestId: 'req_hist_001', vehicleId: v1.id, queryPlate: 'KDA 123X', queryChassis: 'JTEBU3JR3B5045181', requestorMfiId: 'MFI-2847', loanAmountKes: 850000, riskScore: 87, riskLevel: 'CRITICAL', confidence: 0.94, flaggedIssues: JSON.stringify(['LOAN_STACKING_SUSPECT', 'ACTIVE_AUCTION_LISTING']), graphAnalysis: JSON.stringify({ connectedLoans: 3, connectedLenders: 3 }), recommendation: 'REJECT_LOAN', responseTimeMs: 142, createdAt: new Date('2026-08-12T15:41:00') } });
    await db.riskCheck.create({ data: { requestId: 'req_hist_002', vehicleId: v4.id, queryPlate: 'KAE 321M', requestorMfiId: 'MFI-1199', loanAmountKes: 500000, riskScore: 20, riskLevel: 'LOW', confidence: 0.88, flaggedIssues: '[]', graphAnalysis: JSON.stringify({ connectedLoans: 1, connectedLenders: 1 }), recommendation: 'APPROVE_LOAN', responseTimeMs: 67, createdAt: new Date('2026-08-12T14:22:00') } });
    await db.riskCheck.create({ data: { requestId: 'req_hist_003', vehicleId: v5.id, queryPlate: 'KDF 654Z', requestorMfiId: 'MFI-3301', loanAmountKes: 400000, riskScore: 68, riskLevel: 'HIGH', confidence: 0.91, flaggedIssues: JSON.stringify(['MULTIPLE_STORAGE_YARD_APPEARANCES', 'RAPID_RE_PLEDGE']), graphAnalysis: JSON.stringify({ connectedLoans: 2, connectedLenders: 2 }), recommendation: 'REVIEW_MANUALLY', responseTimeMs: 189, createdAt: new Date('2026-08-12T11:05:00') } });

    return NextResponse.json({ message: 'Seeded successfully', vehicleCount: 10 });
  } catch (error) {
